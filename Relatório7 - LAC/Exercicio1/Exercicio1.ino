...
void turnOnLeds(string result)
{
	digitalWrite(led13, result[0] - '0');
	digitalWrite(led12, result[1] - '0');
	digitalWrite(led11, result[2] - '0');
	digitalWrite(led10, result[3] - '0');
}
...
void loop()
{
	if (Serial.available() > 3)
	{
		...
		// decode operands
		a = hexToBin(instruction[0]);
		b = hexToBin(instruction[1]);
		
		// decode operation and execute
		turnOnLeds( operate(instruction[2]) );
	}
}